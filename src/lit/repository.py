...
class Repository:
    ...
    def create_lineage(self, *, lineage_id: str, forked_from: str | None = None, title: str = "", description: str = "") -> OperationRecord:
        ...
        with self._mutation(OperationKind.CREATE_LINEAGE.value, message=f"create lineage {normalized}"):
            operation = self._begin_operation(OperationKind.CREATE_LINEAGE, message=f"create lineage {normalized}", lineage_id=normalized)
            try:
                if self.layout.lineage_path(normalized).exists():
                    raise ValueError(f"lineage already exists: {normalized}")
                self._ensure_lineage(normalized, head_revision=self.resolve_revision(forked_from or "HEAD"), forked_from=forked_from or self.current_branch_name(), title=title or normalized, description=description)
            except Exception as error:
                self._finish_operation(operation, status=OperationStatus.FAILED, lineage_id=normalized, message=str(error))
                raise
            return self._finish_operation(operation, status=OperationStatus.SUCCEEDED, revision_id=self.resolve_revision(forked_from or "HEAD"), lineage_id=normalized)

    def promote_lineage(self, *, lineage_id: str, destination_lineage_id: str | None = None, expected_head_revision: str | None = None) -> OperationRecord:
        source = self.get_lineage(lineage_id)
        destination_id = destination_lineage_id or self.current_branch_name() or self.config.default_branch
        ...
        if expected_head_revision is not None and source.head_revision != expected_head_revision:
            raise ValueError(f"lineage {lineage_id} moved to {source.head_revision}; expected {expected_head_revision}")
        with self._mutation(OperationKind.PROMOTE_LINEAGE.value, message=f"promote {lineage_id} to {destination_id}"):
            ...
            try:
                ...
                self._finish_operation(operation, status=OperationStatus.FAILED, revision_id=source.head_revision, lineage_id=destination_id, message=str(error))
                raise
            return self._finish_operation(operation, status=OperationStatus.SUCCEEDED, revision_id=source.head_revision, lineage_id=destination_id)
    ...
    def list_artifacts(self, *, owner_id: str | None = None) -> tuple[ArtifactRecord, ...]:
        ...
        for path in sorted(self.layout.artifacts.glob("*/artifact.json")):
            record = ArtifactRecord.from_dict(read_json(path, default=None))
    ...
    def _append_revision_checkpoint(self, revision_id: str, checkpoint_id: str) -> None:
        ...
        write_json(self.layout.revision_path(revision_id), updated.to_dict(), mutation=self._transaction_writer())
